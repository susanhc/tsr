...
class splitbase():
    ...
    def saveimagepatches(self,resizeimg,subimgname,left,up):
        subimg = copy.deepcopy(resizeimg[up: (up + self.subsize), left: (left + self.subsize)])
    ...
    def savepatches(self, resizeimg, objects, subimgname, left, up, right, down):
        ...
        for obj in objects:
            ...
            if (half_iou == 1):
                print('*******')
                f_out=codecs.open(outdir, 'w', self.code)
...
if __name__ == '__main__':
    ...
    #split = splitbase(r'example',r'examplesplit')
    split = splitbase(r'tsr',r'tsrsplit')
    split.splitdata(1)